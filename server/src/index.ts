...
app.post('/ingest', async (req, res) => {
  try {
    ...
    // A single malformed or adversarial request must never take down the
    // service this exists to keep available — that failure mode already
    // happened once (see ruleEngine.ts's Math.max fix) and is exactly the
    // outcome an attacker wants from the tool meant to stop them.
    console.error('[ingest] unexpected error, responding 500 instead of crashing:', err);
